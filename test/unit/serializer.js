...
describe('Serializer', () => {
  ...
  describe('serialize', () => {
    it('should calculate the message type and call _serialize', () => {
      ...
      const datas = ['data', 5, null, Buffer.alloc(0), {}, true];
      ...
      expect(spy.calledWith('event', datas[2], Serializer.MT_DATA, 1, Serializer.DT_EMPTY)).to.be.true;
      expect(spy.calledWith('event', datas[3], Serializer.MT_DATA, 1, Serializer.DT_BINARY)).to.be.true;
      expect(spy.calledWith('event', serializer._objectSerializer(datas[4]), Serializer.MT_DATA, 1, Serializer.DT_OBJECT)).to.be.true;
      expect(spy.calledWith('event', datas[5] ? 1 : 0, Serializer.MT_DATA, 1, Serializer.DT_BOOLEAN)).to.be.true;
    });
  });
});
