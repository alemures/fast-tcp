...
Sock.prototype._handleDataStream = function (msg) {
  var stream = this._streams[msg.event];
  var _this = this;
  ...
  if (!stream) {
    ...
    this._streams[msg.event] = stream;
    this._superEmit(msg.event, stream);
  }

  if (!stream.push(msg.data)) {
    this._socket.pause();
  }
};
