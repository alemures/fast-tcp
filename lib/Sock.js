...
const MAX_MESSAGE_ID = Math.pow(2, 32) - 1;
 ...
 * @fires Sock#error
 */
module.exports = class Sock extends EventEmitter {
  constructor(serializer, opts) {
    ...
    this._shouldReconnect = ut.isBoolean(opts.reconnect) ? opts.reconnect : Sock.RECONNECT;

    this._reconnectInterval = ut.isNumber(opts.reconnectInterval) ? opts.reconnectInterval :
        Sock.RECONNECT_INTERVAL;

    this._autoConnect = ut.isBoolean(opts.autoConnect) ? opts.autoConnect : Sock.AUTO_CONNECT;

    this._useQueue = ut.isBoolean(opts.useQueue) ? opts.useQueue : Sock.USE_QUEUE;

    this._queueSize = ut.isNumber(opts.queueSize) ? opts.queueSize : Sock.QUEUE_SIZE;
  ...
  _send(event, data, mt, opts) {
    opts = opts !== undefined ? opts : {};
    const messageId = opts.messageId || this._nextMessageId();

    if (opts.cb !== undefined) {
      this._acks[messageId] = opts.cb;
    }

    const buff = this._serializer.serialize(event, data, mt, messageId);
  ...
  _onMessage(msg) {
    switch (msg.mt) {
      ...
      case Serializer.MT_DATA_STREAM_OPEN:
        let readStream = this._openDataStream(msg);
  ...
  _transmitDataStream(msg) {
    const readStream = this._streams[msg.messageId];
  ...
  _ackCallback(messageId) {
    return (data) => {
      this._send('', data, Serializer.MT_ACK, { messageId: messageId });
    };
  }
