...
class Sock extends EventEmitter {
   ...
   */
  _sendStream(event, data, mt, ack) {
    const opts = { messageId: this._nextMessageId(), ack };

    this._send(event, data, mt, opts);

    const writeStream = new Writable({
      write: (chunk, encoding, cb) => {
        const sendResult = this._send(event, chunk, Serializer.MT_DATA_STREAM, opts);
        switch (sendResult) {
          case Sock.SEND_OK:
            cb();
            break;
  ...
  _removeFromQueue(messageId) {
    this._queue = this._queue.filter(buffer => Serializer._getMessageId(buffer) !== messageId);
  }
