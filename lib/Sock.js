...
class Sock extends EventEmitter {
   ...
   * @return {String}
   */
  static _serializeEvent(event, target, except) {
    if (target !== undefined) {
      event += `${Sock.EVENT_SEPARATOR}${target.join(Sock.LIST_SEPARATOR)}`;
    }

    if (except !== undefined) {
      event += `${Sock.EVENT_SEPARATOR}${except.join(Sock.LIST_SEPARATOR)}`;
    }
   ...
   * @return {Object}
   */
  static _deserializeEvent(event) {
    const splitted = event.split(Sock.EVENT_SEPARATOR);
    return {
      event: splitted[0],
      target: splitted[1] !== undefined ? splitted[1].split(Sock.LIST_SEPARATOR) : null,
      except: splitted[2] !== undefined ? splitted[2].split(Sock.LIST_SEPARATOR) : null
    };
  }
}
