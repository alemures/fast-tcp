...
class Sock extends EventEmitter {
   ...
   * @param {Function} [opts.cb] The callback.
  ...
  _send(event, data, mt, opts) {
    ...
    if (opts.cb !== undefined) {
      this._acks.set(messageId, opts.cb);
      if (opts.timeout !== undefined) {
        this._setAckTimer(messageId, opts.timeout);
      }
    }
  ...
  _sendStream(event, data, mt, cb) {
    const opts = { messageId: this._nextMessageId(), cb };
    ...
    switch (msg.mt) {
      ...
      case Serializer.MT_ACK:
        if (this._acks.has(msg.messageId)) {
          ...
          this._clearAckTimer(msg.messageId);
        }
        break;
      default:
   ...
   */
  _setAckTimer(messageId, timeout) {
    this._acksTimers.set(messageId, setTimeout(() => {
      this._acksTimers.delete(messageId);
  ...
  _clearAckTimer(messageId) {
    clearTimeout(this._acksTimers.get(messageId));
    this._acksTimers.delete(messageId);
  }
