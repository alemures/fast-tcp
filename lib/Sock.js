...
class Sock extends EventEmitter {
  ...
  constructor(serializer, opts) {
    ...
    this._opts.reconnect = Util.isBoolean(opts.reconnect) ? opts.reconnect : Sock.RECONNECT;
    this._opts.reconnectInterval = Util.isNumber(opts.reconnectInterval) ? opts.reconnectInterval :
      Sock.RECONNECT_INTERVAL;
    this._opts.autoConnect = Util.isBoolean(opts.autoConnect) ? opts.autoConnect :
      Sock.AUTO_CONNECT;
    this._opts.useQueue = Util.isBoolean(opts.useQueue) ? opts.useQueue : Sock.USE_QUEUE;
    this._opts.queueSize = Util.isNumber(opts.queueSize) ? opts.queueSize : Sock.QUEUE_SIZE;
  ...
  static get SEND_QUEUED() {
    return 2;
  }

  static get SEND_QUEUED_IN_NODE() {
    return 3;
  }
   ...
   * @param {*} data The data.
   * @param {Number} mt The message type.
   * @param {Object} [opts] The send options.
   * @param {Number} [opts.messageId] The message id to use.
   * @param {Number} [opts.timeout] The ack timeout.
   * @param {Function} [opts.ack] The ack callback.
   * @return {Number} 1 entire message was flushed successfully to the kernel buffer, 2
   *                  message queued, 3 message queued in node, 4 message discarded.
   */
  _send(event, data, mt, opts) {
    opts = opts !== undefined ? opts : {};

    const messageId = opts.messageId !== undefined ? opts.messageId : this._nextMessageId();
    const buffer = this._serializer.serialize(event, data, mt, messageId);

    if (opts.ack !== undefined) {
      this._acks.set(messageId, opts.ack);
      if (opts.timeout !== undefined) {
        this._setAckTimeout(messageId, opts.timeout);
      }
    }

    if (this._connected) {
      return this._nodeSocket.write(buffer) ? Sock.SEND_OK : Sock.SEND_QUEUED_IN_NODE;
    } else if (this._opts.useQueue) {
      if (this._queue.length < this._opts.queueSize) {
        this._queue.push(buffer);
        return Sock.SEND_QUEUED;
      }

      const eventName = Util.deserializeEventName(event);
      /**
       * The message queue is full, the message is rejected.
       *
       * @event Sock#queue_full
       */
      super.emit('queue_full', eventName, data);
    }

    return Sock.SEND_DISCARDED;
  }
   ...
   * @param {*} data The data.
   * @param {Number} mt The message type.
   * @param {Object} [opts] The sendStream options.
   * @param {Number} [opts.timeout] The ack timeout.
   * @param {Function} [opts.ack] The ack callback.
   * @return {stream.Writable}
   */
  _sendStream(event, data, mt, opts) {
    opts = opts !== undefined ? opts : {};
    opts.messageId = this._nextMessageId();

    this._send(event, data, mt, opts);

    const writeStream = new Writable({
      write: (chunk, encoding, cb) => {
        const sendResult = this._send(event, chunk, Serializer.MT_DATA_STREAM, {
        ...
        switch (sendResult) {
          case Sock.SEND_OK:
            cb();
            break;
          case Sock.SEND_QUEUED:
            this.once('drain', cb);
            break;
          case Sock.SEND_QUEUED_IN_NODE:
            this.once('socket_drain', cb);
            break;
          default:
            cb(new Error('broken pipe EPIPE'));
        }
      }
    });

    writeStream.on('finish', () => {
      this._send(event, '', Serializer.MT_DATA_STREAM_CLOSE, { messageId: opts.messageId });
    });
  ...
  _onMessage(msg) {
    ...
    switch (msg.mt) {
      ...
      case Serializer.MT_DATA_STREAM_OPEN:
        ...
        super.emit(msg.event, msg.data, readStream);
        break;
      case Serializer.MT_DATA_STREAM_OPEN_WITH_ACK:
        readStream = this._openDataStream(msg);
        super.emit(msg.event, msg.data, readStream, this._buildAckCallback(msg.messageId));
        break;
      ...
      case Serializer.MT_DATA_WITH_ACK:
        super.emit(msg.event, msg.data, this._buildAckCallback(msg.messageId));
        break;
      case Serializer.MT_ACK:
        if (this._acks.has(msg.messageId)) {
          this._acks.get(msg.messageId)(undefined, msg.data);
          this._acks.delete(msg.messageId);
          this._clearAckTimeout(msg.messageId);
        }
        break;
      default:
  ...
  _flushQueue() {
    this._queue.forEach(buffer => this._nodeSocket.write(buffer));
    this._queue = [];
  }
   ...
   */
  _setAckTimeout(messageId, timeout) {
    this._acksTimeouts.set(messageId, setTimeout(() => {
      this._acksTimeouts.delete(messageId);
      this._acks.get(messageId)(new Error('ack TIMEOUT'));
      this._acks.delete(messageId);
    }, timeout));
  }
   ...
   * @param {Number} messageId
   */
  _clearAckTimeout(messageId) {
    clearTimeout(this._acksTimeouts.get(messageId));
    this._acksTimeouts.delete(messageId);
  }
