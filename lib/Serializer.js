...
Serializer.prototype.serialize = function (event, data, mt, messageId) {
  ...
  if (typeof data === 'object') {
    return this._serialize(event, JSON.stringify(data), mt, messageId, Serializer.DT_JSON);
  }
...
Serializer.prototype._serialize = function (event, data, mt, messageId, dt) {
  ...
  var dataLength = dt === Serializer.DT_STRING || dt === Serializer.DT_JSON ?
      Buffer.byteLength(data) :
      dt === Serializer.DT_BINARY ? data.length :
  ...
  if (dt === Serializer.DT_STRING || dt === Serializer.DT_JSON) {
