...
class Serializer {
    ...
    switch (dt) {
      ...
      case Serializer.DT_OBJECT:
        data = this._objectDeserializer(
          ...
          Util.deserializeEventName(event)
        );
        break;
      case Serializer.DT_BINARY:
  ...
  _parseData(event, data, dt) {
    switch (dt) {
      case Serializer.DT_OBJECT:
        return this._objectSerializer(data, Util.deserializeEventName(event));
      case Serializer.DT_BOOLEAN:
        return data ? 1 : 0;
      default:
        return data;
  ...
  static _getMessageId(buff) {
    return buff.readUInt32LE(Serializer.BYTES_MESSAGE_LENGTH + Serializer.BYTES_VERSION +
      Serializer.BYTES_FLAGS + Serializer.BYTES_DATA_TYPE + Serializer.BYTES_MESSAGE_TYPE);
  }
