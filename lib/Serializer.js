module.exports = class Serializer {
  constructor(opts) {
    this._objectSerializer = opts.objectSerializer !== undefined ?
        opts.objectSerializer : this._defaultObjectSerializer;
    this._objectDeserializer = opts.objectDeserializer !== undefined ?
        opts.objectDeserializer : this._defaultObjectDeserializer;
  }
  ...
  _serialize(event, data, mt, messageId, dt) {
    const eventLength = Buffer.byteLength(event);
    const dataLength = this._getDataLength(data, dt);
    ...
    const buff = new Buffer(4 + messageLength);
    let offset = 0;
  ...
  _defaultObjectSerializer(data, event) {
    return new Buffer(JSON.stringify(data));
  }

  _defaultObjectDeserializer(data, event) {
