...
Serializer.prototype.serialize = function (event, data, mt, messageId) {
  switch (typeof data) {
    ...
    case 'object':
      if (data === null) {
        return this._serialize(event, null, mt, messageId, Serializer.DT_EMPTY);
      } else if (data instanceof Buffer) {
        return this._serialize(event, data, mt, messageId, Serializer.DT_BINARY);
      }

      return this._serialize(event, this._objectSerializer(data, event), mt, messageId,
          Serializer.DT_OBJECT);
    case 'boolean':
      return this._serialize(event, data ? 1 : 0, mt, messageId, Serializer.DT_BOOLEAN);
    default:
      return this._serialize(event, null, mt, messageId, Serializer.DT_EMPTY);
  }
};
...
Serializer.prototype.deserialize = function (buff) {
  ...
  if (version !== Serializer.VERSION) {
    throw ut.error('Message version ' + version + ' and Serializer version ' +
...
Serializer.prototype._serialize = function (event, data, mt, messageId, dt) {
  ...
  var buff = Buffer.allocUnsafe(4 + messageLength);
  var offset = 0;
