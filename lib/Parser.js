...
function writeBuffer(name, data) {
	...
	// Total length
	buff.writeUInt32BE(payloadLength, offset);
	offset += 4;

	// Name
	buff.writeUInt32BE(nameLength, offset);
	offset += 4;
	...
	// Data
	buff.writeUInt32BE(dataLength, offset);
	offset += 4;
...
function readBuffer(buff) {
	...
	var payloadLength = buff.readUInt32BE(offset);
	offset += 4;

	nameLength = buff.readUInt32BE(offset);
	offset += 4;

	name = buff.toString('utf8', offset, offset + nameLength);
	offset += nameLength;

	dataLength = buff.readUInt32BE(offset);
	offset += 4;
