...
class Server extends EventEmitter {
   ...
   * binary data. If no sockets or rooms are provided, the stream
   * will be broadcasted to all connected sockets.
   *
   * @param {String} event The event name.
   * @param {String|Number|Object|Buffer|Boolean} data The data to send.
  ...
  join(room, socketId) {
    ...
    roomArr.forEach(roomName => this._join(roomName, socketId));
  }
  ...
  leave(room, socketId) {
    const roomArr = Util.isString(room) ? [room] : room;
    roomArr.forEach(roomName => this._leave(roomName, socketId));
  }
   ...
   */
  leaveAll(socketId) {
    this.rooms.forEach((sockets, room) => this._leave(room, socketId));
  }
  ...
  _emit(event, data, opts) {
    if (opts.sockets !== undefined) {
      this._emitToSockets(event, data, opts.sockets);
    } else if (opts.rooms !== undefined) {
  ...
  _emitToSockets(event, data, socketIds) {
    ...
    rooms.forEach((room) => {
      ...
      if (sockets !== undefined) {
        sockets.forEach((socket) => {
          if (Server._socketNotExcepted(socket.id, except)) {
            socket.emit(event, data);
          }
        });
      }
  ...
  _stream(event, data, opts) {
    if (opts.sockets !== undefined) {
      return this._streamToSockets(event, data, opts.sockets);
    }
  ...
  _streamToSockets(event, data, socketIds) {
   ...
   * @param {String[]} [except]
   * @return {Boolean} true if socket is not excepted
   * @private
   */
  static _socketNotExcepted(socketId, except) {
    if (except !== undefined) {
      return except.indexOf(socketId) === -1;
    }
