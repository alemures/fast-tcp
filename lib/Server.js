...
Server.prototype.emitExclude = function(event, message, excludedSocketId) {
  var socketId;
  for (socketId in this.sockets) {
...
Server.prototype.join = function(socket, room) {
  var sockets;
...
Server.prototype.leave = function(socket, room) {
  var sockets = this.rooms[room];
  if (sockets !== undefined) {
...
Server.prototype._bindEventsSocket = function(sock) {
  var socket = new Socket({}, sock, this);
  ...
  socket._internalEmit('', socket.id, Serializer.MT_REGISTER);
};
