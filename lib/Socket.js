...
Socket.prototype._emit = function (event, data, opts, cb) {
  ...
  if (socketIds.length + rooms.length === 0 && !broadcast) {
    if (cb !== null) {
      this._send(event, data, Serializer.MT_DATA_WITH_ACK, { cb: cb });
    } else {
      this._send(event, data, Serializer.MT_DATA);
    }
  }
};
...
Socket.prototype._stream = function (event, data, opts, cb) {
  if (cb !== null) {
    return this._sendStream(event, data, Serializer.MT_DATA_STREAM_OPEN_WITH_ACK, { cb: cb });
  }
