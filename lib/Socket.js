 ...
 *                                  will be destroyed. If timeout is 0, then the existing idle
 *                                  timeout is disabled.
 ...
 *                                  serialization protocols for the data type "object" or even use
 *                                  other known protocols like "Protocol Buffers" or  "MessagePack".
 * @constructor
 * @augments Sock
 ...
 */
Socket.prototype.stream = function (event, data) {
  var _this = this;
  var streamOpts = { messageId: this._nextMessageId() };

  this._send(event, data, Serializer.MT_DATA_STREAM_OPEN, streamOpts);

  var writeStream = new Writable({
    write: function (chunk, encoding, cb) {
      if (_this._send(event, chunk, Serializer.MT_DATA_STREAM, streamOpts)) {
        cb();
      } else {
  ...
  writeStream.on('finish', function () {
    _this._send(event, '', Serializer.MT_DATA_STREAM_CLOSE, streamOpts);
  });
