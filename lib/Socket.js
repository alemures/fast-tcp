...
class Socket extends Sock {
   ...
   */
  join(room) {
    room = ut.isString(room) ? room : room.join(Sock.LIST_SEPARATOR);
  ...
  _emit(event, data, opts, cb) {
    ...
    if (socketIds.length > 0) {
      flushedData = this._send(
        this._serializeEvent(event, socketIds),
        data,
    ...
    if (rooms.length > 0) {
      flushedData = this._send(
        this._serializeEvent(event, rooms),
        data,
  ...
  _stream(event, data, opts, cb) {
    ...
    if (socketIds.length > 0) {
      return this._sendStream(
        this._serializeEvent(event, socketIds), data,
    ...
    if (rooms.length > 0) {
      return this._sendStream(
        this._serializeEvent(event, rooms), data,
