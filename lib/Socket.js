  ...
  _emit(event, data, opts, cb) {
    ...
    if (broadcast) {
      flushedData = this._send(
        this._serializeEvent(event, undefined, except),
        data,
    ...
    if (rooms.length > 0) {
      flushedData = this._send(
        this._serializeEvent(event, rooms, except),
        data,
  ...
  _stream(event, data, opts, cb) {
    ...
    if (broadcast) {
      return this._sendStream(
        this._serializeEvent(event, undefined, except), data,
    ...
    if (rooms.length > 0) {
      return this._sendStream(
        this._serializeEvent(event, rooms, except), data,
