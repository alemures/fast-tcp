  ...
  _emit(event, data, opts, cb) {
    ...
    if (broadcast) {
      flushedData = this._send(
        Sock._serializeEvent(event, undefined, except),
        data,
    ...
    if (socketIds.length > 0) {
      flushedData = this._send(
        Sock._serializeEvent(event, socketIds),
        data,
    ...
    if (rooms.length > 0) {
      flushedData = this._send(
        Sock._serializeEvent(event, rooms, except),
        data,
  ...
  _stream(event, data, opts, cb) {
    ...
    if (broadcast) {
      return this._sendStream(
        Sock._serializeEvent(event, undefined, except), data,
    ...
    if (socketIds.length > 0) {
      return this._sendStream(
        Sock._serializeEvent(event, socketIds), data,
    ...
    if (rooms.length > 0) {
      return this._sendStream(
        Sock._serializeEvent(event, rooms, except), data,
