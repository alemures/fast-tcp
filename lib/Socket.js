...
Socket.prototype.stream = function (event, data) {
  ...
  _this._send(event, data, Serializer.MT_DATA_STREAM_OPEN);

  var writeStream = new Writable({
    write: function (chunk, encoding, cb) {
      if (_this._send(event, chunk, Serializer.MT_DATA_STREAM)) {
        cb();
      } else {
  ...
  writeStream.on('finish', function () {
    _this._send(event, '', Serializer.MT_DATA_STREAM_CLOSE);
  });
