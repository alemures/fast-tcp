...
class SocketServ extends Sock {
  ...
  _emit(event, data, opts, cb) {
    const socketIds = ut.isArray(opts.sockets) ? opts.sockets : ut.isString(opts.sockets) ? [opts.sockets] : [];
    const rooms = ut.isArray(opts.rooms) ? opts.rooms : ut.isString(opts.rooms) ? [opts.rooms] : [];
    ...
    if (rooms.length > 0) {
      this._server.emit(event, data, { rooms, except: [this.id] });
    }
  ...
  _stream(event, data, opts, cb) {
    ...
    if (rooms.length > 0) {
      return this._server.stream(event, data, { rooms, except: [this.id] });
    }
  ...
  _msgListener(msg) {
    ...
    switch (msg.mt) {
      ...
      case Serializer.MT_DATA_TO_ROOM:
        eventObj = this._deserializeEvent(msg.event);
        this.emit(eventObj.event, msg.data, { rooms: eventObj.target });
        break;
      ...
      case Serializer.MT_DATA_STREAM_OPEN_TO_ROOM:
        eventObj = this._deserializeEvent(msg.event);
        readStream = this._openDataStream(msg);
        writableStream = this.stream(eventObj.event, msg.data, { rooms: eventObj.target });
        readStream.pipe(writableStream);
        break;
