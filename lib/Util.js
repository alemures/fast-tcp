class Util {
   ...
   * @return {Object}
   */
  static checkEmitOpts(opts, ack) {
    ack = Util.isFunction(opts) ? opts : ack;
  ...
  static serializeEvent(event, target, except) {
    ...
    if (except !== undefined) {
      event += target === undefined ? Util.EVENT_SEPARATOR : '';
  ...
  /**
   * Faster version of Util.deserializeEvent to get only the event name.
   * @param {String} event The serialized event.
   * @return {String}
   */
  static deserializeEventName(event) {
    const sepPos = event.indexOf(Util.EVENT_SEPARATOR);
    if (sepPos > -1) {
      return event.substring(0, sepPos);
    }
    return event;
  }
